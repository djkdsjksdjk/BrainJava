		...
		//1단계 : DB 연결을 위한 커넥션  인터페이스
		 ...
		 //try ~ catch문에서 DB연결중에 예외가 발생하는지를 검사
	        try {
	            ...
	            rs = stmt.executeQuery("select Cname, address, pnumber from Custinfo");
	           ...
	           while(rs.next()) {
	            ...
	            String pnumber = rs.getString(3); //rs.getString("pnumber");
	            
	            System.out.println(Cname+ "---" + address + "---"+ pnumber);
	           }
	            //4단계 : DB연결을 종료한다.
	            conn.close();
	        }
